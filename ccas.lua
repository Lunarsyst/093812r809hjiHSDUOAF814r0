...
local ArcWeapons = {
    ["Grenade Launcher"]=true,["Ultimatum"]=true,["Iron Bomber"]=true,["Loose Cannon"]=true,
    ["Loch-n-Load"]=true,["Huntsman"]=true,["Flare Gun"]=true,["Maverick"]=true,
    ["Detonator"]=true,["Milk Pistol"]=true,["Syringe Crossbow"]=true,
    ["Rescue Ranger"]=true,["Apollo"]=true,["Twin-Turbolence"]=true,
}
...
local BackstabWeapons = {
    ...
    ["Karambit"]=true,["Golden Knife"]=true,
}
...
------------------------------------------------------------
-- HELPER: Obsidian-compatible notify wrapper
------------------------------------------------------------
...
local function GetSilentAimTarget(playerData)
    ...
    if aimTargets["Players"] then
        for _,pd in ipairs(playerData) do
            ...
            if (sortMode == "Closest to Mouse" and dist <= fov and dist < bestDist) or
               (sortMode == "Closest Distance" and dist < bestDist) then
    ...
    if aimTargets["Sentry"] then
        for _,v in pairs(cachedSentries) do
            ...
            if (sortMode == "Closest to Mouse" and d <= fov and d < bestDist) or
               (sortMode == "Closest Distance" and d < bestDist) then bestDist=d; bestPart=pp; bestPlayer=nil end
        end
    end
    if aimTargets["Stickybomb"] then
        ...
        if dest then for _,v in pairs(dest:GetChildren()) do
            if v.Name:match("stickybomb$") and not v.Name:match(LocalPlayer.Name) then
                ...
                if (sortMode == "Closest to Mouse" and d <= fov and d < bestDist) or
                   (sortMode == "Closest Distance" and d < bestDist) then bestDist=d; bestPart=p; bestPlayer=nil end
            end
        end end
    end
    if bestPart then
...
do
    local PG = Tabs.Aimbot:AddLeftGroupbox("Prediction Visuals", "activity")
...
do
    local TG = Tabs.Aimbot:AddLeftGroupbox("Triggerbot", "mouse-pointer")
...
do
    local RG = Tabs.Aimbot:AddRightGroupbox("Ragebot", "zap")
    RG:AddToggle("RagebotToggle", { Text="Enable Ragebot", Default=false })
        :AddKeyPicker("RagebotKey", { Default="T", Mode="Toggle", Text="Ragebot", SyncToggleState=true })
    Toggles.RagebotToggle:OnChanged(function()
        Config.Ragebot.Enabled = Toggles.RagebotToggle.Value
        if Toggles.RagebotToggle.Value then Notify("Ragebot is obvious!", 4) end
    end)
...
do
    local SN = Tabs.Aimbot:AddRightGroupbox("Sniper", "scope")
    SN:AddLabel("ADS Keybind"):AddKeyPicker("ADSBind", { Default="G", Mode="Toggle", Text="ADS",
        Callback=function(v) S.AdsEnabled=v; if not v and S.ads then S.ads.Value=false end end })
    SN:AddSlider("ADSModifier", { Text="ADS Modifier", Default=1, Min=0.1, Max=3, Rounding=2,
        Callback=function(v) S.ADSMultiplier=v; if S.AdsEnabled and S.adsmodifier then S.adsmodifier.Value=v end end })
end

do
    local NG = Tabs.Aimbot:AddRightGroupbox("Notifications", "bell")
...
do
    local TrG = Tabs.Aimbot:AddRightGroupbox("Weapon Tracers", "ruler")
...
do
    ...
    local VT = CTB2:AddTab("Visible Chams")
...
do
    local PSG = Tabs.Visuals:AddRightGroupbox("Prediction Visual Style", "palette")
...
do
    local MR = Tabs.Misc:AddRightGroupbox("Automation", "cpu")
...
do
    ...
    EL:AddLabel("Anti-Aim"):AddKeyPicker("AAKeybind", { Default="J", Mode="Toggle", Text="Anti-Aim",
        Callback=function(v) Config.AntiAim.Enabled=v end })
    EL:AddDropdown("AAMode", { Values={"jitter","backwards","spin"}, Default="jitter", Text="AA Mode",
    ...
    EL:AddToggle("WallbangToggle", { Text="Wallbang", Default=false,
        Callback=function(v) Config.Wallbang.Enable=v; if v then Notify("Wallbang is obvious!", 4) end end })
    ...
    EL:AddToggle("SpeedToggle", { Text="Speed", Default=false })
        :AddKeyPicker("SpeedKey", { Default="N", Mode="Toggle", Text="Speed", SyncToggleState=true })
...
do
    local ER = Tabs.Exploits:AddRightGroupbox("VIP / Other", "crown")
    ER:AddToggle("NoVoiceCooldown", { Text="No Voice Cooldown", Default=false,
        Callback=function(v) pcall(function() ReplicatedStorage.VIPSettings.NoVoiceCooldown.Value=v end) end })
    ER:AddToggle("ThirdPersonMode", { Text="Third Person", Default=false,
...
local function CheckAutoUber()
    ...
    pcall(function()
        local gun = char:FindFirstChild("Gun"); if gun then
            local boop = gun:FindFirstChild("Boop"); if boop then
                local weaponName = boop.Value
                if weaponName=="Medigun" or weaponName=="Kritzkrieg" or weaponName=="Blood Doctor" or weaponName=="Rejuvenator" or weaponName=="The Vaccinator" then
                    local pg = LocalPlayer:FindFirstChild("PlayerGui"); if pg then
                        local gui = pg:FindFirstChild("GUI"); if gui then
                            local client = gui:FindFirstChild("Client"); if client then
                                local llv = client:FindFirstChild("LegacyLocalVariables"); if llv then
                                    local uber = llv:FindFirstChild("uber"); if uber and uber.Value >= 100 then uberReady=true end
                                end end end end end end end
    end)
    if not uberReady then return end
    local myHealth = hum.Health; local myMaxHealth = hum.MaxHealth
    local myHealthPercent = (myHealth/myMaxHealth)*100; local threshold = Config.AutoUber.HealthPercent
    if Config.AutoUber.Condition=="Self" or Config.AutoUber.Condition=="Both" then
        if myHealthPercent <= threshold then RightClick(); return end
    end
        ...
        pcall(function()
            local pg=LocalPlayer:FindFirstChild("PlayerGui"); if pg then
                local gui=pg:FindFirstChild("GUI"); if gui then
                    local client=gui:FindFirstChild("Client"); if client then
                        local llv=client:FindFirstChild("LegacyLocalVariables"); if llv then
                            local healTarget=llv:FindFirstChild("healtarget"); if healTarget and healTarget.Value ~= "" then
                                ...
                                if targetPlayer and targetPlayer.Character then
                                    ...
                                    if targetHum then
                                        ...
                                        if targetHealthPercent <= threshold then RightClick(); return end
    ...
    for _,v in workspace.Ray_Ignore:GetChildren() do
        if v:GetAttribute("ProjectileType") and v:GetAttribute("Team") then
            if v:GetAttribute("Team") ~= LocalPlayer.Status.Team.Value then
                local Vec, OnScr = Camera:WorldToViewportPoint(v.Position)
                if OnScr and (v.Position-lhrp.Position).Magnitude <= 13 then RightClick(); return end
            end
        end
    end
...
local function RunSilentAimLogic()
    ...
    if isMobileMode then S.silentAimKeyActive=true
    else S.silentAimKeyActive=Options.SilentAimKey:GetState() end
        ...
        if not BlacklistedWeapons[weapon] then
            ...
            if target then
                if isMobileMode then
                    pcall(function()
                        local LLV=LocalPlayer.PlayerGui.GUI.Client.LegacyLocalVariables
                        if not LLV.Held1.Value then LLV.Held1.Value=true; S.shooting=true; S.lastShotTime=tick() end
                    end)
                else
...
local function RunAutoWarp(playerData)
    ...
    for _,pd in ipairs(playerData) do
        ...
        if dot > 0 then
            ...
            S.lastWarpTime=tick()
            Notify("Warped behind " .. pd.Player.Name, 1.5)
            break
        end
    end
end
...
local function RunAutoBackstab(playerData)
    ...
    for _,pd in ipairs(playerData) do
        ...
        if toT:Dot(pd.HRP.CFrame.LookVector) > 0.3 then
            foundTarget=true
            if S.lastBackstabTarget ~= pd.Player then S.lastBackstabTarget=pd.Player; Notify("Backstab target: " .. pd.Player.Name, 2) end
            if Toggles.AutoBackstabAimArms and Toggles.AutoBackstabAimArms.Value then
    ...
    if not foundTarget and S.lastBackstabTarget then Notify("Target lost", 1.5); S.lastBackstabTarget=nil end
end
...
local function RunAutoMelee(playerData)
    ...
    local myWeapon=GetLocalWeapon(); if not MeleeWeapons[myWeapon] then return end
    local lc=GetLocalCharacter(); local lh=lc and GetHRP(lc); if not lh then return end
    local meleeMode=Options.AutoMeleeMode and Options.AutoMeleeMode.Value or "Rage"
    local meleeRange=meleeMode=="Demoknight" and MELEE_RANGE_DEMOKNIGHT or MELEE_RANGE_RAGE
    local foundTarget=false
    for _,pd in ipairs(playerData) do
        ...
        foundTarget=true; if S.lastMeleeTarget ~= pd.Player then S.lastMeleeTarget=pd.Player end
        if not S.shooting then
            VirtualInputManager:SendMouseButtonEvent(0,0,0,true,game,0); S.shooting=true; S.lastShotTime=tick()
        elseif tick()-S.lastShotTime >= 0.15 then
            VirtualInputManager:SendMouseButtonEvent(0,0,0,false,game,0)
...
LogService.MessageOut:Connect(function(message)
    ...
    if message:find("HIT_DEBUG") then
        if tick()-S.lastHitDebugNotif > 2 then
            Notify("Shot rejected by server — ping may be too high", 3)
...
do
    ...
    MenuGroup:AddDropdown("DPIDropdown", {
        ...
        Callback = function(value)
            value = value:gsub("%%",""); local DPI = tonumber(value); Library:SetDPIScale(DPI)
        end,
    })
...
print("[Aegis Loaded!]")
print("67420 children in africa got killed because of you btw")
